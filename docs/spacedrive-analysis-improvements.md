...
- Pure Rust walkdir-based scanner (Zig FFI removed in `fb22c40`)

**Weaknesses identified**:
1. `extract_metadata_batch` uses `rayon::par_iter()` for >= 20 files (threshold-based parallelism added in task-272)
...
| Impact | Complexity | Area | Status |
|--------|-----------|------|--------|
| **H** | **L** | Scanner | **Done** (task-272) |
...
**Status**: Implemented in task-272. `metadata.rs` now uses `rayon::par_iter()` for batches >= 20 files.
...
| Impact | Complexity | Area | Status |
|--------|-----------|------|--------|
| **M** | **L** | Scanner | **Partial** (task-272) |

**Problem**: `to_track_metadata()` (in both `watcher.rs` and `scanner/commands.rs`) calls `compute_content_hash()` synchronously for every track during scanning. SHA-256 of full file contents is I/O-intensive.
...
**Status**: Partially addressed in task-272 — `to_db_metadata_with_hash()` now accepts a precomputed hash to avoid redundant SHA-256 when the hash was already computed during reconciliation. Full deferral not yet implemented.

#### 6. Scoped Fingerprint Queries in Watcher
| Impact | Complexity | Area | Status |
|--------|-----------|------|--------|
| **M** | **L** | Watcher | **Done** (task-272) |

**Problem**: `trigger_rescan` in `watcher.rs` queries ALL library fingerprints (`SELECT filepath, file_mtime_ns, file_size FROM library`) regardless of which folder is being scanned.
...
**Status**: Implemented in task-272. `get_fingerprints_for_paths()` in `db/library.rs` scopes queries at the SQL level. Scanner `scan_paths_to_library` uses `get_scoped_fingerprints()` instead of fetch-all + in-memory filter. Watcher path still uses the old pattern — to be updated separately.
...
| Impact | Complexity | Area | Status |
|--------|-----------|------|--------|
| **M** | **M** | Library Rendering | **Partial** (task-272) |
...
**Status**: Partially addressed in task-272 — bulk delete commands (`library_delete_tracks`, `library_delete_all`, `library_purge_missing`) now emit `LibraryUpdatedEvent` with track IDs. Frontend `removeTracksLocally()` filters `filteredTracks` directly (O(n)) instead of re-running `applyFilters()` (O(n log n)). Debounced scan events and optimistic UI also implemented. Full incremental patching for small updates not yet done.
...
| Area | File | Line(s) | Issue | Status |
|------|------|---------|-------|--------|
| All-tracks fetch | `app/frontend/js/stores/library.js` | ~245 | `limit: 999999` | Open |
| All-tracks fetch | `app/frontend/js/components/albums-browser.js` | — | Removed; uses `allTracks` store | **Fixed** (task-272) |
| All-tracks fetch | `app/frontend/js/components/artists-browser.js` | — | Removed; uses `allTracks` store | **Fixed** (task-272) |
| Full rescan on event | `crates/mt-tauri/src/watcher.rs` | ~284 | `trigger_rescan` on every FS event | Open |
| Global fingerprint query | `crates/mt-tauri/src/watcher.rs` | ~378 | Unscoped `SELECT FROM library` | Open (scanner path **fixed**) |
| Sync content hash | `crates/mt-tauri/src/watcher.rs` | ~668 | `compute_content_hash` in `to_track_metadata` | Open |
| Sync content hash | `crates/mt-tauri/src/scanner/commands.rs` | ~339 | `compute_content_hash` in `to_db_metadata_with_hash` | **Mitigated** — reuses hash from reconciliation |
| Hardcoded extensions | `crates/mt-tauri/src/watcher.rs` | ~246 | Duplicates `AUDIO_EXTENSIONS` | Open |
| Client-side re-sort | `app/frontend/js/stores/library.js` | ~727 | `applyFilters()` re-sorts entire array | **Mitigated** — `removeTracksLocally` skips `applyFilters` |
| Parallel metadata | `crates/mt-tauri/src/scanner/metadata.rs` | ~210 | `extract_metadata_batch` | **Fixed** — uses `rayon::par_iter` for >= 20 files |
| Serialization-only tests | `crates/mt-tauri/src/library/commands.rs` | ~474 | Tests verify JSON, not DB logic | Open |
| Empty event payloads | `crates/mt-tauri/src/scanner/commands.rs` | ~274 | `LibraryUpdatedEvent::added(vec![])` | Open (bulk deletes now emit with IDs) |
