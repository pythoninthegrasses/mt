...
<!-- SECTION:NOTES:BEGIN -->
## Fix: Artist-name-start matching only (no word-boundary matching)

**Root cause:** The original `jumpToMatchingArtist()` matched at word boundaries within artist names, so typing "la" matched "Laulan" inside "Konami Kukeiha Club - Yoann Laulan" before reaching "The La's".

**Fix:** Matching now only checks if the artist name starts with the query:
1. **Primary**: Artist name with ignore words stripped (e.g., "The La's" → strip "the " → "la's" starts with "la")
2. **Fallback**: Raw artist name starts with query (e.g., user types "the" → matches "The Beatles")

Word-boundary matching was removed entirely. For substring searches, the search bar serves that purpose.

Added "..." (ellipsis) to `DEFAULT_SORT_IGNORE_WORDS` for artists like "...And You Will Know Us by the Trail of Dead".
<!-- SECTION:NOTES:END -->
